...
#
# Generated on 2026-08-28T17:49:31Z by kres 6e80e64.
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-debug/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-debug/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
COPY ./debug_on_127_test.go ./debug_on_127_test.go
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-debug/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-debug/go/pkg dis-vulncheck -tool=false ./...
